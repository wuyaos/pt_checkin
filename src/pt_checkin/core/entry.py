...
class SignInEntry:
    ...
    def __init__(self, title: str, url: str = ""):
        self.data: Dict[str, Any] = {"title": title, "url": url}
        self.failed = False
        self.reason = ""
    ...
    def fail(self, reason: str, error_type: str = "general") -> None:
        ...
        existing_priority = error_priority.get(
            getattr(self, "_error_type", "general"), 1
        )
    ...
    def fail_with_prefix(self, reason: str, error_type: str = "general") -> None:
        ...
        full_reason = (
            f"{prefix}=> {reason}. ({last_date})"
            if prefix
            else f"{reason}. ({last_date})"
        )
    ...
    def last_date(self) -> str:
        ...
        if cookies_backup_file.is_file():
            try:
                cookies_backup_json = json.loads(
                    cookies_backup_file.read_text(encoding="utf-8")
                )
