...
class SignInStatusManager:
    ...
    def is_signed_today(self, site_name: str) -> bool:
        ...
        return (
            today in self.status_data
            and site_name in self.status_data[today]
            and self.status_data[today][site_name].get("status") == "success"
        )
    ...
    def record_signin_success(
        self,
        site_name: str,
        result: str,
        messages: str = "",
        details: str = "",
        signin_type: str = "签到成功",
    ) -> None:
        """记录签到成功"""
        today = self.get_today_key()
    ...
    def record_signin_failed(self, site_name: str, reason: str) -> None:
        ...
        if site_name in self.status_data[today]:
            current_failed_count = self.status_data[today][site_name].get(
                "failed_count", 0
            )
            # 如果当前状态是成功，重置失败次数
    ...
    def clear_site_status(
        self, site_name: str, keep_failed_count: bool = False
    ) -> None:
        """清除站点今日状态（用于强制重新签到）"""
        today = self.get_today_key()
        if today in self.status_data and site_name in self.status_data[today]:
            if keep_failed_count:
                ...
                if failed_count > 0:
                    self.status_data[today][site_name] = {"failed_count": failed_count}
            else:
                del self.status_data[today][site_name]
            self.save_status()
    ...
    def should_skip_due_to_failures(
        self, site_name: str, max_failed_attempts: int, retry_interval_hours: int = 2
    ) -> tuple[bool, str]:
        """检查是否应该因为失败次数过多而跳过站点"""
        ...
        if site_status and site_status.get("status") == "failed":
            try:
                ...
                if hours_since_failure < retry_interval_hours:
                    return (
                        True,
                        f"连续失败{failed_count}次，{retry_interval_hours}小时内不再尝试",
                    )
                else:
                    # 超过重试间隔，可以重新尝试
                    return False, ""
    ...
    def get_today_summary(self) -> Dict[str, Any]:
        ...
        success_count = sum(
            1 for site in sites_data.values() if site.get("status") == "success"
        )
        failed_count = sum(
            1 for site in sites_data.values() if site.get("status") == "failed"
        )
