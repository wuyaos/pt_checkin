...
from ..base.sign_in import SignState, Work, check_final_state, check_sign_in_state
...
class MainClass(Attendance):
    ...
    def sign_in_build_workflow(self, entry: SignInEntry, config: dict) -> list[Work]:
        return [
            ...
            Work(
                ...
                succeed_regex=["签到成功", "今天已签到，请勿重复刷新"],
                assert_state=(check_final_state, SignState.SUCCEED),
            ),
        ]

    def sign_in_by_userid(
        self,
        entry: SignInEntry,
        config: dict,
        work: Work,
    ...
    def details_selector(self) -> dict:
        ...
        net_utils.dict_merge(
            selector,
            {
                ...
                "details": {
                    "seeding": {"regex": r"活动:.*?(\d+)"},
                    "leeching": {"regex": (r"活动:.*?(\d+).*?(\d+)", 2)},
                },
            },
        )
        return selector
    ...
    def get_nexusphp_messages(self, entry: SignInEntry, config: dict, **kwargs) -> None:
        super().get_nexusphp_messages(
            entry, config, unread_elements_selector='td > i[alt*="Unread"]'
        )
