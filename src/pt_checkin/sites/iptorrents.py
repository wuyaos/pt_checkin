    ...
    def details_selector(self) -> dict:
        return {
            ...
            "detail_sources": {
                "default": {
                    "link": "/user.php?u={}",
                    "elements": {"bar": "div.stats", "table": "table#body table.t1"},
                }
            },
            "details": {
                "uploaded": {"regex": "Uploaded.+?([\\d.]+ [ZEPTGMK]?B)"},
                "downloaded": {"regex": "Downloaded.+?([\\d.]+ [ZEPTGMK]?B)"},
                "share_ratio": {
                    "regex": r"Ratio (-|[\d,.]+)",
                    "handle": handle_infinite,
                },
                "points": {"regex": r"Bonus Points\s+([\d,.]+)"},
                "join_date": {
                    "regex": "Join date\\s*?(\\d{4}-\\d{2}-\\d{2})",
                    "handle": handle_join_date,
                },
                "seeding": {"regex": "Seeding([\\d,]+)"},
                "leeching": {"regex": "Leeching([\\d,]+)"},
                "hr": None,
            },
        }
