    ...
    def sign_in_build_workflow(self, entry: SignInEntry, config: dict) -> list[Work]:
        return [
            ...
            Work(
                ...
                succeed_regex=[
                    '{"state":"success","signindays":"\\d+","integral":"\\d+"}'
                ],
                fail_regex="验证码错误",
    ...
    def sign_in_by_ocr(
            ...
            try:
                ...
                if img_response.status_code != 200:
                    entry.fail_with_prefix(
                        f"Get image failed: HTTP {img_response.status_code}"
                    )
                    return None
        ...
        try:
            ...
            if len(img_content) < 100:  # 图片数据太小，可能有问题
                entry.fail_with_prefix(
                    f"Image data too small: {len(img_content)} bytes"
                )
                return None
        ...
        if not entry.failed:
            if len(code) >= 4:  # 验证码长度可能不是固定6位
                ...
                return self.request(entry, "post", signin_url, config, data=data)
        return None
