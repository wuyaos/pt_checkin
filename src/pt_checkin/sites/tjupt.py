        ...
        net_utils.dict_merge(
            selector,
            {
                ...
                "details": {
                    "downloaded": None,
                    "share_ratio": None,
                    "seeding": {"regex": "种子数合计.*?(\\d+)"},
                    "leeching": {"regex": "种子数合计.*?\\d+\\D+(\\d+)"},
                    "hr": {"regex": "H&R.*?(\\d+)", "handle": self.handle_hr},
                },
            },
        )
        return selector

    def sign_in_by_douban(
    ...
    def get_answer(self, entry, config, captcha_img_url, answers):
        ...
        for value, answer in answers:
            ...
            movies = requests.get(
                ...
                headers={"user-agent": config.get("user-agent")},
            ).json()
            if len(movies) == 0:
                logger.info(
                    f"https://movie.douban.com/j/subject_suggest?q={answer} length: 0"
                )
            for movie in movies:
                ...
                logger.info(
                    f'{movie.get("title")} url:{movie.get("img")} score: {score}'
                )
                if score > 0.9:
                    ...
                    question_file.write_text(
                        json.dumps(question_json, indent=4, ensure_ascii=False),
                        encoding="utf-8",
                    )
                    return value
                sleep(5)
        ...
        logger.info(
            json.dumps(
                {img_name: {"hash": captcha_img_hash, "answer": ""}},
                indent=4,
                ensure_ascii=False,
            )
        )
