...
class MainClass(NexusPHP):
    ...
    def sign_in_build_workflow(self, entry: SignInEntry, config: dict) -> list[Work]:
        ...
        succeed_regex = [
            self.USERNAME_REGEX.format(username=site_config.get("username"))
            + self.SUCCEED_REGEX,
    ...
    def build_data(
        ...
        logger.debug(
            "attempts: {} / {}, url: {}",
            self.times,
            ocr_config.get("retry"),
            urljoin(entry["url"], img_url),
        )
        data = {}
        found = False
        if images := self.get_image(
            entry, config, img_url, ocr_config.get("char_count")
        ):
                ...
                for key, regex in work.data.items():
                    if key == "regex_keys":
                        for regex_key in regex:
                            ...
                            for captcha, value in regex_key_search:
                                if answer_list := list(
                                    ...
                                    split_value, partial_ratio = process.extractOne(
                                        oct_text, answer_list, scorer=fuzz.partial_ratio
                                    )
                                else:
                                    partial_ratio = 0
                        ...
                        if not (
                            ...
                            entry.fail_with_prefix(
                                "Cannot find key: {}, url: {}".format(key, work.url)
                            )
                            return None
        ...
        if not found:
            ...
            reload__net_state = check_network_state(
                entry, real_reload_url, reload_response
            )
    ...
    def get_image(
        self, entry: SignInEntry, config: dict, img_url: str, char_count: int
    ) -> tuple | None:
        image_list = []
        checked_list = []
    ...
    def get_new_image(self, entry: SignInEntry, img_url: str) -> Image.Image | None:
        ...
        if (
            base_img_response is None
            or base_img_response.status_code != 200
            or base_img_response.url
            == urljoin(entry["url"], "/pic/trans.gif?debug=NIM")
        ):
            return None
        ...
        net_utils.dict_merge(
            selector,
            {
                "details": {
                    ...
                    "points": {"regex": r"UCoin.*?\d+\(([\d,.]+)"},
                    "seeding": {"regex": r"客户端.*?: \d+ \((\d+)"},
                    "leeching": {"regex": r"客户端.*?: \d+ \(\d+ (\d+)"},
                    "hr": None,
                }
            },
        )
