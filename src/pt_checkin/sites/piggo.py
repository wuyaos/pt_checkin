...
from ..base.request import NetworkState, cf_detected
    ...
    def details_selector(self) -> dict:
        ...
        net_utils.dict_merge(
            selector,
            {
                ...
                "details": {"points": {"regex": (r"(做种积分).*?([\d,.]+)", 2)}},
            },
        )
        return selector
    ...
    def request(
        ...
        **kwargs,
        ...
        try:
            ...
            entry["session_cookie"] = " ".join(
                list(map(lambda x: f"{x[0]}={x[1]};", self.session.cookies.items()))
            )
            return response
