...
class MainClass(Unit3D):
    ...
    def sign_in_build_login_data(self, login: dict, last_content: str) -> dict:
        m = re.search(
            r'name="(?P<name>.+?)" value="(?P<value>.+?)" />\s*<button type="submit"',
            last_content,
        )
        return {
            "_token": re.search(
                r'(?<=name="_token" value=").+?(?=")', last_content
            ).group(),
            "username": login["username"],
            "password": login["password"],
            "remember": "on",
            "_captcha": re.search(
                r'(?<=name="_captcha" value=").+?(?=")', last_content
            ).group(),
            "_username": "",
    ...
    def sign_in_build_workflow(self, entry: SignInEntry, config: dict) -> list[Work]:
        return [
            Work(
                ...
                method=self.sign_in_by_get,
                succeed_regex=[
                    ('<a href="https://www.torrentseeds.org/users/(.*?)">', 1)
                ],
