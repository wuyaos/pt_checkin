...
class MainClass(Unit3D):
    ...
    def details_selector(self) -> dict:
        return {
            ...
            "details": {
                "uploaded": {"regex": ("(上传|Upload).+?([\\d.]+ ?[ZEPTGMK]?iB)", 2)},
                "downloaded": {
                    "regex": ("(下载|Download).+?([\\d.]+ ?[ZEPTGMK]?iB)", 2)
                },
                "share_ratio": {"regex": ("(分享率|Ratio).+?([\\d.]+)", 2)},
                "points": {
                    "regex": ("(魔力|BP:).+?(\\d[\\d,. ]*)", 2),
                ...
                "seeding": {"regex": (r"(\d+)\s*?\d*%\s*?(做种|Active Seeds)", 1)},
                "leeching": {"regex": (r"(\d+)\s*?(吸血|Active Downloads)", 1)},
                "hr": {"regex": ("(警告|Warnings).+?(\\d+)", 2)},
            },
        }
