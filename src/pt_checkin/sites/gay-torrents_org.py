...
class MainClass(XBTIT):
    ...
    USER_CLASSES: Final = {"uploaded": [268435456000], "share_ratio": [2]}
    ...
    def details_selector(self) -> dict:
        ...
        net_utils.dict_merge(
            selector,
            {
                ...
                "details": {
                    "uploaded": {"regex": r"Uploaded\s*([\d.]+ ([ZEPTGMK]i)?B)"},
                    "downloaded": {"regex": r"Downloaded\s*([\d.]+ ([ZEPTGMK]i)?B)"},
                    "share_ratio": {
                        "regex": r"Ratio\s*(---|[\d.]+)",
                    ...
                    "seeding": {"regex": r"Active:\s*(\d+)\s*/\s*\d+"},
                    "leeching": {"regex": r"Active:\s*\d+\s*/\s*(\d+)"},
                    "hr": None,
                },
            },
        )
