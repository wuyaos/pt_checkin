    ...
    def sign_in_build_workflow(self, entry: SignInEntry, config: dict) -> list[Work]:
        return [
            ...
            Work(
                ...
                data=self.DATA,
                succeed_regex=[
                    "您已连续签到\\d+天，奖励\\d+积分，明天继续签到将获得\\d+积分奖励。"
                ],
        ...
        net_utils.dict_merge(
            selector,
            {
                ...
                "details": {
                    ...
                    "downloaded": {
                        "regex": (
                            "(下[载載]量|Downloaded).+?([\\d.]+ ?[ZEPTGMk]?i?B)",
                            2,
                        ),
                        "handle": self.handle_size,
                    },
                    "points": {"regex": "积分.*?([\\d,.]+)"},
                    "seeding": {"regex": "做种活动.*?(\\d+)"},
                    "leeching": {"regex": "做种活动.*?\\d+\\D+(\\d+)"},
                    "hr": {"regex": "HP.*?(\\d+)", "handle": self.handle_hr},
                },
            },
        )
        return selector
