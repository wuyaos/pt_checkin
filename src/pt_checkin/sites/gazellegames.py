...
class MainClass(Gazelle):
            ...
            selector,
            {
                "detail_sources": {
                    ...
                    "achievements": {
                        ...
                        "elements": {"total_point": "#content > div[class=linkbox]"},
                    },
                },
                "details": {
                    "points": {"regex": "Total Points: (\\d+)"},
                    "hr": {"regex": "Hit 'n' Runs\">(\\d+)"},
                },
            },
        )
        return selector
    ...
    def get_details(self, entry: SignInEntry, config: dict) -> None:
        ...
        if not (key and name):
            ...
            return
        params = {"request": "user", "key": key, "name": name}
        ...
        entry["details"] = {
            ...
            "share_ratio": handle_infinite(
                str(
                    details_response_json.get("response").get("stats").get("ratio") or 0
                ).replace(",", "")
            ),
            "points": str(
                details_response_json.get("response")
                .get("achievements")
                .get("totalPoints")
                or 0
            ).replace(",", ""),
            "seeding": str(
                details_response_json.get("response").get("community").get("seeding")
                or 0
            ).replace(",", ""),
            "leeching": str(
                details_response_json.get("response").get("community").get("leeching")
                or 0
            ).replace(",", ""),
            "hr": str(
                details_response_json.get("response").get("personal").get("hnrs") or 0
            ).replace(",", ""),
        }
    ...
    def get_api_response_json(self, entry: SignInEntry, params: dict) -> dict | None:
        ...
        network_state = check_network_state(
            entry, api_response.request.url, api_response
        )
        if network_state != NetworkState.SUCCEED:
            return None
