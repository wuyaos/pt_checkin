...
class Unit3D(PrivateTorrent, ABC):
    ...
    def details_selector(self) -> dict:
        selector = {
            ...
            "details": {
                "uploaded": {"regex": (r"(上传|Upload).+?([\d.]+.?[ZEPTGMK]?iB)", 2)},
                "downloaded": {
                    "regex": (r"(下载|Download).+?([\d.]+.?[ZEPTGMK]?iB)", 2)
                },
                "share_ratio": {"regex": (r"(分享率|Ratio).+?([\d.]+)", 2)},
                "points": {
                ...
                "seeding": {"regex": (r"(做种|Seeding).+?(\d+)", 2)},
                "leeching": {"regex": (r"(吸血|Leeching).+?(\d+)", 2)},
                "hr": {"regex": (r"(警告|Warnings).+?(\d+)", 2)},
            },
        }
        return selector
