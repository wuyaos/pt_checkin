...
class Gazelle(PrivateTorrent, ABC):
        ...
        return {
            ...
            "detail_sources": {
                "default": {
                    "link": "/user.php?id={}",
                    "elements": {
                        "table": "#content > div > div.sidebar > div.box.box_info.box_userinfo_stats > ul"
                    },
                }
            },
            "details": {
                ...
                "points": {"regex": (r"(Gold|积分|Bonus|Credits|Nips).*?([\d,.]+)", 2)},
                "join_date": {
                    "regex": ("(Joined|加入时间).*?(.*?)(ago|前|Last seen)", 2),
                    "handle": self.handle_join_date,
                },
                "seeding": {"regex": r"[Ss]eeding.+?([\d,]+)"},
                "leeching": {"regex": r"[Ll]eeching.+?([\d,]+)"},
                "hr": None,
            },
        }

    def get_gazelle_message(
        self,
        entry: SignInEntry,
        config: dict,
        message_body_selector: str = 'div[id*="message"]',
    ) -> None:
        ...
        ).select("tr.unreadpm > td > strong > a")
        failed = False
        for unread_element in unread_elements:
            ...
            if network_state != NetworkState.SUCCEED:
                ...
                ).select_one(message_body_selector)
                if body_element:
    ...
    def handle_join_date(self, value: str) -> datetime.date:
        ...
        return (
            ...
            - datetime.timedelta(days=year * 365 + month * 31 + week * 7)
        ).date()
