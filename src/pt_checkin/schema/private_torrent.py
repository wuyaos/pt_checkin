...
from ..base.request import NetworkState, RequestHandler, check_network_state
from ..base.sign_in import SignIn, Work, check_state
...
class PrivateTorrent(SignIn, Detail, Message, ABC):
    ...
    def request(
        self, entry: SignInEntry, method: str, url: str, config: dict = None, **kwargs
    ):
    ...
    def sign_in(self, entry: SignInEntry, config: dict) -> None:
        ...
        for work in workflow:
            ...
            work.response_urls = list(
                map(
                    lambda response_url: urljoin(entry["url"], response_url),
                    work.response_urls,
                )
            )
    ...
    def get_user_id(
        ...
        logger.error(
            f'site: {entry["site_name"]} User id not found. content: {base_content}'
        )
        return None
                    ...
                    if sel:
                        if details_info := soup.select_one(sel):
                            details_text += (
                                ...
                                else details_info.text
                            )
                        else:
                            entry.fail_with_prefix(f"Element: {name} not found.")
                            logger.error(
                                "site: {} element: {} not found, selector: {}, soup: {}",
                                entry["site_name"],
                                name,
                                sel,
                                soup,
