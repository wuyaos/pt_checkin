...
class XBTIT(PrivateTorrent, ABC):
    ...
    def details_selector(self) -> dict:
        return {
            ...
            "details": {
                "uploaded": {"regex": r"↑.([\d,.]+ [ZEPTGMK]?iB)"},
                "downloaded": {"regex": r"↓.([\d,.]+ [ZEPTGMK]?iB)"},
                "share_ratio": {"regex": r"Ratio: ([\d.]+)"},
                "points": {"regex": r"Bonus Points:.+?([\d,.]+)"},
                "join_date": {
                    "regex": r"Joined on.*?(\d{2}/\d{2}/\d{4})",
                    "handle": self.handle_join_date,
                },
                "seeding": {"regex": r"Seeding (\d+)"},
                "leeching": {"regex": r"Leeching (\d+)"},
                "hr": None,
            },
        }
    ...
    def get_XBTIT_message(
        ...
        entry: SignInEntry,
        config: dict,
        MESSAGES_URL_REGEX: str = "usercp\\.php\\?uid=\\d+&do=pm&action=list",
    ) -> None:
        ...
        message_elements = BeautifulSoup(
            net_utils.decode(message_box_response, "html.parser")
        ).select("tr > td.lista:nth-child(1)")
        unread_elements = filter(
            lambda elements: elements.get_text() == "no", message_elements
        )
        failed = False
        for unread_element in unread_elements:
            td = (
                unread_element.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling
            )
            title = td.text
            href = td.a.get("href")
