...
class NexusPHP(PrivateTorrent, ABC):
    ...
    def details_selector(self) -> dict:
        return {
            ...
            "details": {
                ...
                "points": {"regex": (r"(魔力|Bonus|Bônus).*?([\d,.]+)", 2)},
                "join_date": {
                    "regex": (
                        r"(加入日期|注册日期|Join.date|Data de Entrada).*?(\d{4}-\d{2}-\d{2})",
                        2,
                    ),
                },
                "seeding": {
    ...
    def get_nexusphp_messages(
        ...
        message_box_network_state = check_network_state(
        ...
        for unread_element in unread_elements:
            ...
            if message_network_state != NetworkState.SUCCEED:
                ...
                if body_element := BeautifulSoup(
                    net_utils.decode(message_response), "html.parser"
...
class Attendance(AttendanceHR, ABC):
    ...
    def details_selector(self) -> dict:
        ...
        net_utils.dict_merge(selector, {"details": {"hr": None}})
        return selector
...
class BakatestHR(NexusPHP, ABC):
    def sign_in_build_workflow(self, entry: SignInEntry, config: dict) -> list[Work]:
        return [
            ...
            Work(
                ...
                succeed_regex=[
                    "连续.*天签到,获得.*点魔力值|今天已经签过到了\\(已连续.*天签到\\)"
                ],
    ...
    def sign_in_by_question(
        self, entry: SignInEntry, config: dict, work: Work, last_content: str = None
    ) -> None:
        question_element = BeautifulSoup(last_content, "html.parser").select_one(
            'input[name="questionid"]'
        )
        if question_element:
            question_id = question_element.get("value")

            question_file = Path(__file__).parent.parent.joinpath(
            ...
            choice_elements = BeautifulSoup(last_content, "html.parser").select(
                'input[name="choice[]"]'
            )
            choices = []
            ...
            if (
                local_answer
                and len(local_answer) == len([i for i in local_answer if i in choices])
            ...
            for answer in answer_list:
                ...
                state = check_sign_in_state(
                    entry, work, response, net_utils.decode(response)
                )
