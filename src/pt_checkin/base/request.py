...
def check_network_state(
    ...
    if response is None or (check_content and content is None):
        entry.fail_with_prefix(
            NetworkState.NETWORK_ERROR.value.format(
                url=urls[0], error="Response is None"
            )
        )
    ...
    if cf_detected(response):
        entry.fail_with_prefix(
            NetworkState.CLOUDFLARE_BLOCKED.value.format(url=urls[0])
        )
        return NetworkState.CLOUDFLARE_BLOCKED

    if response.url not in urls:
        entry.fail_with_prefix(
            NetworkState.URL_REDIRECT.value.format(
                original_url=urls[0], redirect_url=response.url
            )
        )
...
def cf_detected(response: Response) -> bool:
    if response is not None and hasattr(response, "text"):
        return bool(
            re.search(r"security by.*Cloudflare</a>", response.text, flags=re.DOTALL)
        )
    return False
...
class RequestHandler:
    ...
    def request(self, method: str, url: str, **kwargs) -> Response | None:
        ...
        if (
            log_key not in self._last_request_method_log
            or current_time - self._last_request_method_log.get(log_key, 0) > 30
        ):
    ...
    def _determine_request_method(self) -> RequestMethod:
        """根据配置确定请求方式"""
        site_name = self.entry.get("site_name") or UrlUtils.extract_site_name(
            self.entry.get("url", "")
        )
        ...
        try:
            ...
            if cf_detected(response):
                ...
                self.entry.fail_with_prefix(
                    f"url: {url} detected CloudFlare DDoS-GUARD. Consider using FlareSolverr."
                )
    ...
    def _request_with_flaresolverr(
        self, method: str, url: str, **kwargs
    ) -> Response | None:
        """使用FlareSolverr发送请求"""
        try:
