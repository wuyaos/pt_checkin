...
class StandardSelectorBuilder(SelectorBuilder):
    ...
    def with_user_id(
        self, user_id_selector: Optional[str]
    ) -> "StandardSelectorBuilder":
        """设置用户ID选择器"""
        self.overrides["user_id"] = user_id_selector
        return self

    def with_detail_source(
        self, name: str, source_config: Dict[str, Any]
    ) -> "StandardSelectorBuilder":
        """添加详情源配置"""
    ...
    def with_detail(
        self, name: str, detail_config: Dict[str, Any]
    ) -> "StandardSelectorBuilder":
        """添加详情字段配置"""
...
class ConfigDrivenSelectorBuilder(SelectorBuilder):
    ...
    def with_site_config(
        self, site_name: str, config: Dict[str, Any]
    ) -> "ConfigDrivenSelectorBuilder":
        """设置站点特定配置"""
...
class SelectorBuilderFactory:
    ...
    @staticmethod
    def create_standard_builder(
        base_selector: Optional[Dict[str, Any]] = None,
    ) -> StandardSelectorBuilder:
        """创建标准选择器构建器"""
        return StandardSelectorBuilder(base_selector)

    @staticmethod
    def create_config_driven_builder(
        base_selector: Optional[Dict[str, Any]] = None,
    ) -> ConfigDrivenSelectorBuilder:
        """创建配置驱动的选择器构建器"""
    ...
    @staticmethod
    def create_builder_for_site(
        site_name: str, base_selector: Optional[Dict[str, Any]] = None
    ) -> SelectorBuilder:
        """为特定站点创建合适的构建器"""
        # 根据站点特性选择合适的构建器
