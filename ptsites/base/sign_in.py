...
class NetworkErrorReason(Enum):
    ...
    SERVER_LOAD_TOO_HIGH = (
        r'<h3 align=center>(服务器负载过|伺服器負載過)高，'
        r'正在重(试|試)，(请|請)稍(后|後)\.\.\.</h3>'
    )
    CONNECTION_TIMED_OUT = (
        r'<h2 class="text-gray-600 leading-1\.3 text-3xl font-light">'
        r'Connection timed out</h2>'
    )
...
def check_sign_in_state(entry: SignInEntry,
    ...
    if (check_network_state(entry, work, response, content=content,
                            check_content=True) != NetworkState.SUCCEED):
        return SignState.SIGN_IN_FAILED
    if content is None:
        entry.fail_with_prefix(
            SignState.SIGN_IN_FAILED.value.format('content is None'))
        return SignState.SIGN_IN_FAILED
    ...
    for regex in succeed_regex:
        ...
        if succeed_msg := re.search(regex, content):
            entry['result'] = re.sub(
                '<.*?>|&shy;|&nbsp;', '',
                succeed_msg.group(group_index)
            ) + entry.get('result', '') + entry.get('extra_msg', '')
            return SignState.SUCCEED
    ...
    for reason in NetworkErrorReason:
        if re.search(reason.value, content):
            entry.fail_with_prefix(NetworkState.NETWORK_ERROR.value.format(
                url=work.url, error=reason.name.title()))
            return SignState.SIGN_IN_FAILED
    if (assert_state := work.assert_state) and \
            assert_state[1] != SignState.NO_SIGN_IN:
        logger.warning(
            f'没有签到, 正则表达式: {succeed_regex}, 内容: {content}')
...
def check_final_state(entry: SignInEntry,
    ...
    if sign_in_state == SignState.NO_SIGN_IN:
        entry.fail_with_prefix(
            SignState.SIGN_IN_FAILED.value.format('no sign in'))
        return SignState.SIGN_IN_FAILED
