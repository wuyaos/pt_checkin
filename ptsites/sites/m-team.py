...
class MainClass(NexusPHP):
    ...
    def get_details(self, entry: SignInEntry, config: dict) -> None:
        ...
        if not details_response_json:
            return
        my_peer_status_response = self.request(
            entry, 'POST', urljoin(self.URL, self.MY_PEER_STATUS))
        if not my_peer_status_response:
            return
        my_peer_status_response_json = my_peer_status_response.json()
        member_count = details_response_json.get("data").get("memberCount")
        entry['details'] = {
            'uploaded': f'{member_count.get("uploaded") or 0} B'.replace(',', ''),
            'downloaded': f'{member_count.get("downloaded") or 0} B'.replace(',', ''),
            'share_ratio': handle_infinite(
                str(member_count.get("shareRate") or 0).replace(',', '')),
            'points': str(member_count.get("bonus") or 0).replace(',', ''),
            'seeding': str(
                my_peer_status_response_json.get('data').get(
                    'seeder') or 0).replace(',', ''),
            'leeching': str(
                my_peer_status_response_json.get('data').get(
                    'leecher') or 0).replace(',', ''),
            'hr': '*'
        }
    ...
    def sign_in_by_post(self,
        ...
        if last_content is None:
            entry.fail_with_prefix('last_content is None')
            return None
        response = super().sign_in_by_post(entry, config, work, last_content)
        if not response:
            return
        response_json = response.json()
        last_browse = response_json.get('data').get(
            'memberStatus').get('lastBrowse')
        target_date = datetime.datetime.strptime(
            last_browse, '%Y-%m-%d %H:%M:%S')
    ...
    def request(self,
        ...
        if not (site_config := entry.get('site_config')):
            entry.fail_with_prefix('site_config not found')
            return None
        key = site_config.get('key')
        try:
            return super().request(entry, 'POST', url,
