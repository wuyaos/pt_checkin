...
class MainClass(NexusPHP):
    ...
    ANSWER_SELECTOR: Final = ('table.captcha form > table > tbody > '
                              'tr:nth-child(2) > td')
    ...
    def sign_in_by_douban(self, entry: SignInEntry, config: dict, work: Work,
                          last_content: str | None):
        if last_content and (break_match := re.search(self.BREAK_REGEX,
                                                      last_content)):
            ...
            if not response:
                return
            network_state = check_network_state(entry, response.url,
                                                response)
            if not network_state == NetworkState.SUCCEED:
                return
            if not (last_content := common.decode(response)):
                return
        if not last_content:
            return
        soup = BeautifulSoup(last_content, 'lxml')
        captcha_el = soup.select_one(self.IMG_SELECTOR)
        answer_el = soup.select_one(self.ANSWER_SELECTOR)
        if not captcha_el or not answer_el:
            entry.fail_with_prefix('Cannot find captcha element')
            return
        value = list(map(lambda x: x['value'], answer_el.select('input')))
        answer = BeautifulSoup(
            str(answer_el).replace('<br/>', '|'),
            'lxml').text.split('|')[0:-1]
        ...
        if isinstance(src, list):
            src = src[0]
        captcha_img_url = urljoin(self.URL, src)
    ...
    def get_answer(self, entry, config, captcha_img_url, answers):
        ...
        if (captcha_img_response is None
                or captcha_img_response.status_code != 200):
        ...
        for value, answer in answers:
            logger.info(f"问答题答案: {value}, {answer}")
            skip = False
            ...
            if skip:
                ...
                continue
            movies = requests.get(
                f'https://movie.douban.com/j/subject_suggest?q={answer}',
                headers={
            ...
            for movie in movies:
                ...
                if (movie_img_response is None
                        or movie_img_response.status_code != 200):
                ...
                logger.info(
                    (f'{movie.get("title")} url:{movie.get("img")} '
                     f'score: {score}')
                )
                if score > 0.9:
                    ...
                    question_file.write_text(json.dumps(
                        question_json, indent=4, ensure_ascii=False),
                        encoding='utf-8')
                    return value
                sleep(5)
        logger.info(f'图片名称: {captcha_img_url}, 答案: {answers}')
    ...
    def get_messages(self, entry: SignInEntry, config: dict) -> None:
        self.get_nexusphp_messages(entry,
                                   config,
